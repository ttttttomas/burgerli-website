    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.log('[WebSocket] Order ID del mensaje:', data.order_id || data.id_order);
          ...
          if (data.event === 'status_update') {
            ...
            if (messageOrderId === orderId) {
              ...
              console.log(`[WebSocket] ✅ Actualización de estado para orden ${orderId}:`, newStatus);
              ...
              console.log(`[WebSocket] ⚠️ Mensaje para otra orden. Esperado: ${orderId}, Recibido: ${messageOrderId}`);
            }
          } else {
            console.log(`[WebSocket] ℹ️ Evento ignorado: ${data.event}`);
          }
        } catch (err) {
      ...
      ws.onclose = (event) => {
        ...
        if (event.code !== 1000 && reconnectAttemptsRef.current < maxReconnectAttempts) {
          ...
          console.log(`[WebSocket] Reintentando conexión en ${delay}ms (intento ${reconnectAttemptsRef.current + 1}/${maxReconnectAttempts})`);
